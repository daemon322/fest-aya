...
const Tribuna = () => {
  ...
  useEffect(() => {
    ...
    const initThree = () => {
      ...
      // 1. Escena con niebla para profundidad
      ...
      // 3. Renderizador con sombras optimizadas
      const renderer = new THREE.WebGLRenderer({ antialias: true, alpha: true, powerPreference: "high-performance" });
      renderer.setSize(window.innerWidth, window.innerHeight);
      renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
      renderer.shadowMap.enabled = true;
      renderer.shadowMap.type = THREE.PCFSoftShadowMap; // sombras más suaves y rápidas
      ...
      // 5. Construcción del complejo
      const sportsComplex = new THREE.Group();
      complexRef.current = sportsComplex;

      // Piso base (una sola pieza)
      const floorGeo = new THREE.PlaneGeometry(35, 45);
      ...
      // Cancha (azul)
      const courtGeo = new THREE.PlaneGeometry(12, 22);
      ...
      const perimeter = new THREE.Mesh(new THREE.PlaneGeometry(12.2, 22.2), lineMaterial);
      perimeter.rotation.x = -Math.PI / 2;
      perimeter.position.y = 0.075;
      lineGroup.add(perimeter);
      const centerLine = new THREE.Mesh(new THREE.PlaneGeometry(12.2, 0.2), lineMaterial);
      centerLine.rotation.x = -Math.PI / 2;
      centerLine.position.y = 0.09;
      lineGroup.add(centerLine);
      sportsComplex.add(lineGroup);
      ...
      postL.position.set(-6.2, 1.75, 0);
      postL.castShadow = true;
      sportsComplex.add(postL);
      const postR = new THREE.Mesh(postGeo, postMat);
      postR.position.set(6.2, 1.75, 0);
      postR.castShadow = true;
      sportsComplex.add(postR);
      
      // Malla de la red (más realista con textura de rejilla)
      ...
      const netPanel = new THREE.Mesh(new THREE.BoxGeometry(12.5, 1.8, 0.1), netMaterial);
      netPanel.position.set(0, 2.3, 0);
      sportsComplex.add(netPanel);

      // Graderías (simplificadas para rendimiento)
      const createBleachers = (side) => {
        const group = new THREE.Group();
        const steps = 7;
        const bleacherColor = 0x0b6a8d; // Gris azulado oscuro para armonizar
        for(let i = 0; i < steps; i++) {
          const stepGeo = new THREE.BoxGeometry(5, 0.8, 26);
          ...
          if (i === steps - 1) {
            const rail = new THREE.Mesh(new THREE.BoxGeometry(0.1, 2, 26), new THREE.MeshStandardMaterial({ color: 0x4b5563 }));
            rail.position.set(2.5, 1.4, 0);
            group.add(rail);
          }
        }
      ...
      const createVIPRow = (xPos, side) => {
        ...
        for(let i = 0; i < 14; i++) {
          ...
          // Rotar la silla para que mire hacia el centro (0, 0, 0)
          // Si xPos es negativo (izquierda), rotamos 90 grados a la derecha. 
          // Si xPos es positivo (derecha), rotamos -90 grados.
      ...
      const animate = () => {
        ...
        if (complexRef.current) {
          ...
          if (isRotating && activeZone === 'Overview') {
  ...
  // Efecto para cambios de vista (sin reiniciar Three.js)
  useEffect(() => {
    if (!threeInitialized.current) return;
    ...
    if (activeZone === 'VIP') {
      ...
      targetCamPos.current = { x: 25, y: 10, z: -18 };  // Vista desde las gradas norte
      targetRotation.current = { x: 0, y: 0 };        // Complejo sin inclinar
      setIsRotating(false);
    } else {
  ...
  // Clases estáticas para los botones (evita problemas con Tailwind)
          ...
          <div className="hidden lg:block pointer-events-auto bg-black/10 backdrop-blur-xl border border-white/10 p-6 rounded-[2rem] max-w-xs w-full select-none">
            ...
            <div className="space-y-4">
              ...
              <div className="pt-2 border-t border-white/5 flex gap-2">
                ...
                <span className="text-[10px] text-gray-400 leading-tight">Acceso habilitado 1 hora antes del evento principal.</span>
              </div>
            </div>
          </div>
        </div>
