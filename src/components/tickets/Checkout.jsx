...
const validateEmail = (email) => {
  ...
  // Solo se aceptan correos Gmail reales
...
const validateFormData = (data) => {
  ...
  if (!validateEmail(email))           errors.push("Solo se aceptan correos Gmail (@gmail.com)");
  if (!validatePhone(phone))           errors.push("Teléfono no válido");
  if (!validateDocument(document))     errors.push("Documento debe tener entre 6 y 20 caracteres");
...
const Checkout = ({ cart = [], onBack, onComplete }) => {
  ...
  const [refNumber]               = useState(() => Math.random().toString(36).toUpperCase().substring(2, 9));
  ...
  const handleConfirmAndSubmit = async (e) => {
    ...
    if (!recaptchaToken) {
      setFormErrors(["Por favor completa la verificación de seguridad (reCAPTCHA)."]);
      setIsSubmitting(false);
      return;
    }
    ...
    try {
      ...
      if (voucher) {
        ...
        await new Promise((resolve, reject) => {
          reader.onload  = () => { voucherBase64 = reader.result; resolve(); };
      ...
      if (response.ok) {
        ...
        try {
          ...
          const msg = errorData.message || "Error al procesar la reserva. Intenta de nuevo.";
          setFormErrors([msg]);
        } catch {
          setFormErrors(["Error al procesar la reserva. Verifica tu conexión e intenta de nuevo."]);
        }
        recaptchaRef.current?.reset();
      }
    } catch (error) {
      console.error("Error enviando reserva:", error);
      setFormErrors(["Error de conexión. Verifica tu internet e intenta de nuevo."]);
      recaptchaRef.current?.reset();
    } finally {
      setIsSubmitting(false);
    }
    ...
    <div className="flex items-center justify-center space-x-4 mb-16">
      {[1, 2, 3, 4].map((i) => (
        <div key={i} className="flex items-center">
          ...
          {i < 4 && (
            <div className={`w-16 h-[1px] mx-2 ${step > i ? "bg-white" : "bg-white/5"}`} />
          )}
        </div>
      ))}
    </div>
      ...
      <div className="max-w-4xl w-full p-8 pt-20">
        ...
        <div className="flex justify-between items-center mb-12">
          ...
          <div className="text-right">
            <p className="text-[8px] uppercase tracking-[0.5em] text-amber-500 mb-1 font-bold">Checkout Seguro</p>
        ...
        <div className="max-w-2xl mx-auto pb-24">
          ...
          {step === 1 && (
            <div className="animate-in slide-in-from-bottom-8 duration-700 space-y-8">
              <header className="text-center mb-12">
                <h2 className="text-5xl font-serif italic text-white mb-4">Revisión de Reserva</h2>
              ...
              <div className="space-y-4">
                {safeCart.length > 0 ? (
                  safeCart.map((item, idx) => (
                    <div key={idx} className="bg-[#0a0a0a] p-8 rounded-[2rem] border border-white/5 flex justify-between items-center group hover:border-white/10 transition-colors">
                      <div className="space-y-3">
                        <span className="px-3 py-1 bg-amber-500/10 text-amber-500 text-[14px] font-bold uppercase tracking-widest rounded-full">
                          {item.phaseName}
                        </span>
                        <h3 className="text-xl font-bold uppercase tracking-widest text-white">{item.title}</h3>
                      ...
                      <div className="text-right">
                        <p className="text-[12px] text-white/80 uppercase tracking-widest mb-1">Subtotal</p>
                  ...
                  <div className="text-center py-20 bg-[#0a0a0a] rounded-[2rem] border border-white/5">
                    <p className="text-white/20 uppercase tracking-widest text-[10px]">El carrito está vacío</p>
                  </div>
                )}
              </div>
              ...
              <div className="bg-white/[0.02] p-10 rounded-[2.5rem] border border-white/5 mt-10 space-y-6">
                <div className="flex justify-between text-[14px] uppercase tracking-[0.3em] text-white/90">
                  ...
                  <span className="font-mono text-white/80">S/ {subtotal.toFixed(2)}</span>
                </div>
                <div className="pt-8 border-t border-white/10 flex justify-between items-center">
                  <div className="space-y-1">
                    <span className="text-xs font-bold uppercase tracking-[0.5em] text-white">Total Final</span>
                    <p className="text-[9px] text-white/70 uppercase tracking-widest italic">Incluye impuestos y cargos</p>
                  </div>
                  <span className="text-5xl font-mono text-amber-500 tracking-tighter">S/ {subtotal.toFixed(2)}</span>
                </div>
              </div>
          ...
          {step === 2 && (
            <div className="animate-in slide-in-from-bottom-8 duration-700 space-y-10">
              <header className="text-center">
                <h2 className="text-5xl font-serif italic text-white mb-4">Titular</h2>
              ...
              <div className="grid grid-cols-1 gap-6">
                {[
                  { label: "Nombre Completo",     id: "name",     ph: "Ej: Juan Pérez" },
                  { label: "Correo Electrónico",   id: "email",    ph: "tucorreo@gmail.com" },
                  { label: "Teléfono / WhatsApp",  id: "phone",    ph: "+51 999 888 777" },
                  { label: "DNI / Pasaporte",      id: "document", ph: "Número de documento" },
                ].map((input) => (
              ...
              {formErrors.length > 0 && (
                <div className="bg-red-500/10 border border-red-500/20 p-6 rounded-2xl space-y-2">
                  {formErrors.map((error, idx) => (
                    <p key={idx} className="text-[10px] uppercase tracking-widest text-red-400">⚠ {error}</p>
                  ))}
                </div>
              )}
              ...
              <button
                onClick={() => {
                  ...
                  const validation = validateFormData(formData);
                  if (!validation.isValid) { setFormErrors(validation.errors); return; }
                  setFormErrors([]);
                  nextStep();
                }}
          ...
          {step === 3 && (
            <div className="animate-in slide-in-from-bottom-8 duration-700 space-y-10">
              <header className="text-center">
                <h2 className="text-5xl font-serif italic text-white mb-4">Validar Pago</h2>
                <p className="text-[10px] uppercase tracking-[0.4em] text-white/90">Adjunta el comprobante de yapeo</p>
              </header>
              ...
              <div className="bg-amber-500/5 border border-amber-500/20 p-8 rounded-3xl flex items-start gap-5">
                ...
                <div className="space-y-2">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-amber-500">Cuenta a Yapear</p>
              ...
              <div className="relative group">
                ...
                >
                  ...
                  <p className="text-[9px] text-white/20 uppercase tracking-widest">
                    {voucher ? voucher.name : "Arrastra aquí o haz clic · JPG, PNG, WEBP, PDF"}
                  </p>
                </label>
              </div>
          ...
          {step === 4 && (
            <div className="animate-in slide-in-from-bottom-8 duration-700 space-y-10">
              <header className="text-center">
                <h2 className="text-5xl font-serif italic text-white mb-4">Confirmación</h2>
                <p className="text-[10px] uppercase tracking-[0.4em] text-white/90">
              ...
              <div className="space-y-6">
                <div className="bg-[#0a0a0a] p-8 rounded-3xl border border-white/5 space-y-6">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="space-y-2">
                      <p className="text-[10px] font-bold uppercase tracking-widest text-white">Garantía de Autenticidad</p>
                      <p className="text-[11px] leading-relaxed text-white/70">
                        Al confirmar, enviaremos los detalles a tu correo electrónico. Un asesor revisará el
                        comprobante adjunto y habilitará tus códigos QR en un lapso de 12 a 24 horas.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <label className="flex items-center gap-5 p-6 bg-white/[0.02] border border-white/5 rounded-2xl cursor-pointer group hover:bg-white/[0.05] transition-all">
                  ...
                  <div className={`w-6 h-6 rounded border flex items-center justify-center transition-all ${agreed ? "bg-amber-500 border-amber-500" : "border-white/20"}`}>
                    {agreed && <CheckCircle2 size={16} className="text-black" />}
                  </div>
                ...
                <button
                  ...
                  onClick={() => {
                    const validation = validateFormData(formData);
                    if (!validation.isValid) { setFormErrors(validation.errors); return; }
                    if (!formData.name || !formData.email || !formData.phone || !formData.document || !voucher || !agreed) {
          ...
          <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="bg-[#0a0a0a] border border-white/10 rounded-3xl max-w-md w-full mx-4 p-10 space-y-8 animate-in zoom-in-95 duration-300 shadow-2xl">
              <div className="flex justify-between items-start">
                <div className="space-y-2">
                  <h3 className="text-2xl font-serif italic text-white">Confirmar Reserva</h3>
                  <p className="text-[10px] uppercase tracking-[0.3em] text-amber-500">Último paso</p>
                </div>
                <button onClick={() => setShowConfirmDialog(false)} className="text-white/40 hover:text-white transition-colors cursor-pointer">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="bg-white/[0.02] p-6 rounded-2xl border border-white/5 space-y-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="space-y-2">
                    <p className="text-[10px] font-bold uppercase tracking-widest text-white">¿Deseas finalizar tu reserva?</p>
                    <p className="text-[11px] leading-relaxed text-white/70">
                      Se enviará un email a{" "}
                      <span className="text-amber-500 font-mono">{formData.email}</span>{" "}
                      con los detalles de tu reserva. Nuestro equipo la revisará y te enviará la
          ...
          <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-black/90 backdrop-blur-sm animate-in fade-in duration-300 px-4">
            <div className="bg-[#0a0a0a] border border-green-500/30 rounded-3xl max-w-lg w-full mx-auto p-8 sm:p-10 space-y-6 animate-in zoom-in-95 duration-300 shadow-2xl overflow-y-auto max-h-[90vh]">
              ...
              <div className="flex flex-col items-center space-y-4 text-center">
                ...
                <div>
                  <h3 className="text-3xl font-serif italic text-white">¡Reserva Exitosa!</h3>
              ...
              <div className="bg-green-500/5 border border-green-500/20 rounded-2xl px-6 py-4 text-center space-y-1">
                <p className="text-[10px] uppercase tracking-widest text-white/40">Confirmación enviada a</p>
                <p className="text-sm font-mono text-green-400 break-all">{formData.email}</p>
              </div>

              {/* Proceso detallado */}
              <div className="space-y-4">
                <p className="text-[10px] uppercase tracking-[0.3em] text-white/30 font-black">¿Qué sigue ahora?</p>

                <div className="space-y-3">
                  {/* Paso 1 */}
                  <div className="flex gap-4 items-start bg-white/[0.02] border border-white/5 rounded-2xl p-5">
                    <div className="w-7 h-7 rounded-full bg-green-500/20 border border-green-500/40 flex items-center justify-center text-green-400 text-[10px] font-black shrink-0 mt-0.5">1</div>
                    <div className="space-y-1">
                      <p className="text-[11px] font-black uppercase tracking-widest text-white">Revisión de tu comprobante</p>
                      <p className="text-[11px] text-white/50 leading-relaxed">
                        Nuestro equipo verificará el voucher que adjuntaste. Una vez validado recibirás una confirmación en tu correo.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-4 items-start bg-amber-500/[0.04] border border-amber-500/20 rounded-2xl p-5">
                    <div className="w-7 h-7 rounded-full bg-amber-500/20 border border-amber-500/40 flex items-center justify-center text-amber-400 text-[10px] font-black shrink-0 mt-0.5">2</div>
                    <div className="space-y-1">
                      <p className="text-[11px] font-black uppercase tracking-widest text-white">Entrega de tu código QR</p>
                      <p className="text-[11px] text-white/50 leading-relaxed">
                        Tu E-Ticket con código QR será enviado a tu correo{" "}
                        <span className="text-amber-400 font-bold">entre 30 minutos y 1 hora</span>{" "}
                        después de recibir la confirmación de pago.
                      </p>
                      <p className="text-[10px] text-white/30 leading-relaxed mt-1 italic">
                        ⚠ Si hay alta demanda de reservas simultáneas, el tiempo máximo de espera es de{" "}
                        <span className="text-white/50 font-bold">24 horas</span>. Te pedimos paciencia y comprensión.
                      </p>
                    </div>
                  </div>
                  ...
                  {/* Paso 3 */}
                  <div className="flex gap-4 items-start bg-white/[0.02] border border-white/5 rounded-2xl p-5">
                    <div className="w-7 h-7 rounded-full bg-white/10 border border-white/20 flex items-center justify-center text-white/60 text-[10px] font-black shrink-0 mt-0.5">3</div>
                    <div className="space-y-1">
                      <p className="text-[11px] font-black uppercase tracking-widest text-white">Día del evento</p>
                      <p className="text-[11px] text-white/50 leading-relaxed">
                        Presenta tu QR desde el celular con el brillo al máximo. Lleva tu DNI — las entradas son nominativas e intransferibles.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-white/[0.02] border border-white/5 rounded-xl px-5 py-4">
                <p className="text-[10px] text-white/30 leading-relaxed text-center">
                  ...
                  <a href="https://wa.me/51961379018" className="text-amber-500/70 underline">+51 961 379 018</a>.
